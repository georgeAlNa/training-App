...
import 'package:training_app/data/datasource/remote/plans/planToLossWeightData/replacingexerciseloss_data.dart';

abstract class ReplacingExerciseLossController extends GetxController {
  replaceExercise();
}

class ReplacingExerciseLossControllerImp
    extends ReplacingExerciseLossController {
  GlobalKey<FormState> formState = GlobalKey<FormState>();
  StatusRequest statusRequest = StatusRequest.none;
  ReplacingExerciseLossData replacingExerciseLossData =
      ReplacingExerciseLossData(Get.find());
  ...
  replaceExercise() async {
    ...
    if (formState.currentState!.validate()) {
      ...
      update();
      var response = await replacingExerciseLossData.postData(
          token!, planiId.text, oldExerciseiId.text, newExerciseiId.text);
      print('response ===== $response');
      statusRequest = handlingData(response);
      if (StatusRequest.success == statusRequest) {
        if (response['message'] == 'Plane Update Successfully') {
